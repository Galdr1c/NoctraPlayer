...
public partial class PlayerTimeline : UserControl
{
    ...
    {
        if (_vm is null || _vm.IsLiveContent || e.Key is not (Key.Left or Key.Right)) return;
        var command = e.Key == Key.Left ? _vm.SkipBackwardCommand : _vm.SkipForwardCommand;
    ...
    private void Detach()
    {
        if (_isAttached && _vm is not null) _vm.PropertyChanged -= OnVmPropertyChanged;
        _isAttached = false;
        _isDragging = false;
    ...
    private void OnPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        if (_vm.StartSeekingCommand.CanExecute(null)) _vm.StartSeekingCommand.Execute(null);
    ...
    private void CommitSeek()
    {
        ...
        if (_vm.SeekCommand.CanExecute(target)) _vm.SeekCommand.Execute(target);
    }
    ...
    private void UpdateProgress()
    {
        ...
        var displayedPosition = _isDragging ? _previewPosition : _vm.Position;
        var playedProgress = _vm.IsLiveContent ? _vm.LiveProgramProgress / 100d : _vm.Duration > 0 ? displayedPosition / _vm.Duration : 0;
        playedProgress = Math.Clamp(playedProgress, 0, 1);
        var bufferedProgress = !_vm.IsLiveContent && _vm.Duration > 0 ? _vm.BufferedPosition / _vm.Duration : 0;
        bufferedProgress = Math.Clamp(Math.Max(bufferedProgress, playedProgress), 0, 1);
        ...
        BufferBar.IsVisible = !_vm.IsLiveContent && !_vm.IsDownloadedPlayback && bufferedWidth > playedWidth + 1;
        ...
        var thumbX = Math.Clamp(TrackGrid.Bounds.X + playedWidth - (thumbWidth / 2d), 0, Math.Max(0, RootGrid.Bounds.Width - thumbWidth));
