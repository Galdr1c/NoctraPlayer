...
public partial class MainWindow : Window
{
    ...
    private async void Context_AddToMyList_Click(object? sender, RoutedEventArgs e)
    {
        ...
        await _mainViewModel.AddToMyListCommand.ExecuteAsync(media);
    }
    ...
    private async void Context_ToggleFavorite_Click(object? sender, RoutedEventArgs e)
    {
        ...
        await _mainViewModel.ToggleFavoriteCommand.ExecuteAsync(media);
    }
    ...
    private async void Context_RemoveFromMyList_Click(object? sender, RoutedEventArgs e)
    {
        ...
        await _mainViewModel.RemoveFromMyListCommand.ExecuteAsync(media);
    }
    ...
    private async void Context_RemoveFromFavorites_Click(object? sender, RoutedEventArgs e)
    {
        ...
        await _mainViewModel.RemoveFromFavoritesCommand.ExecuteAsync(media);
    }

    private static object? ResolveContextMedia(MenuItem menuItem)
    {
        if (menuItem.CommandParameter is Channel || menuItem.CommandParameter is Series)
        {
            return menuItem.CommandParameter;
        }

        if (menuItem.Tag is Channel || menuItem.Tag is Series)
        {
            return menuItem.Tag;
        }

        if (menuItem.DataContext is Channel || menuItem.DataContext is Series)
        {
            return menuItem.DataContext;
        }

        if (menuItem.Parent is ContextMenu contextMenu &&
            contextMenu.PlacementTarget is StyledElement placementTarget &&
            ...
            ownerMenu.PlacementTarget is Control placementControl)
        {
            ...
            while (parent != null)
            {
                if (parent is StyledElement styled &&
                    (styled.DataContext is Channel || styled.DataContext is Series))
                {
                    return styled.DataContext;
                }
