...
- [Yeni] **Rahatsız Etmeyen Gösterim Standardı**: Hatırlatıcı ana uygulama ekranı açıldıktan sonra aktif oturumda 3 dakika gecikmeyle gösterilir. Bu state profil bazlı değil global tutulur, yani profil değiştirince tekrar tekrar sormaz. `Sonra`, `Bir daha sorma` ve başarılı Store açma tercihleri global ayarlara yazılır.
- [Düzeltildi] **Video Overlay Üstünde Kesin Blok**: Video oynatıcı yüzeyi, video overlay veya PiP aktifken review popup gösterilmez. Eğer popup zamanı kullanıcı video izlerken gelirse servis 30 dakikaya kadar 30 saniyelik aralıklarla uygun ana ekranı bekler; video üstünde dialog açmaz.
- [Düzeltildi] **Yalnız Ana Menü Yüzeyinde Gösterim**: Review popup artık yalnızca ana menü içerik yüzeyi gerçekten görünürken gösterilir. Header, ana içerik ve status bar görünür değilse; video player, PiP, Series detail, global loading veya başka dialog aktifse popup açılmaz. Boş ana sayfada `IsLoading`/`IsChannelLoading` takılı kalsa bile ana menü yüzeyi uygunsa prompt beklemeye devam edip kaybolmaz.
- [Düzeltildi] **Bağımsız/Topmost Popup Davranışı**: Review popup `Topmost` olmaktan çıkarıldı ve ana pencereye owner olarak bağlı kalır; fullscreen/video yüzeyinin üstüne bağımsız pencere gibi çıkmaz. Genel `DialogWindow` topmost davranışı korunur, böylece Continue Watching/resume gibi mevcut kritik prompt'ların altta kalma davranışı değişmez.
- [UI] **Hata Dialog Başlığı Kısaltıldı ve Sarma Eklendi**: Microsoft Store açılamadı hata başlığı kısa hale getirildi ve genel dialog başlığı wrap destekleyecek şekilde güncellendi; uzun başlıkların kart dışına taşması engellendi.
- [Düzeltildi] **Debug/Unpackaged Ortamda Popup'ın Hiç Çıkmaması**: Store ProductId veya paket PFN yoksa popup artık tamamen gizlenmez. Kullanıcı `Değerlendir` derse Store açılamadı mesajı gösterilir ve kalıcı kapatma yapılmaz; akış `Sonra` seçilmiş gibi 3 gün ertelenir. Store paketinde gerçek `NoctraStoreProductId`, `NoctraStoreReviewLaunchUri` veya PFN varsa Microsoft Store değerlendirme sayfası açılır ve başarılı Store açma sonrası popup tekrar gelmez.
- [Not] **Review Popup ile Diğer Prompt'lar Ayrıldı**: Review popup'ın ana pencereye bağlı ve `Topmost=False` çalışması yalnızca `ReviewPromptWindow` için geçerlidir. `DialogWindow` kullanan Continue Watching/resume, hata, confirmation ve benzeri mevcut dialog'lar eski topmost davranışını korur.
- [Yeni] **Kullanıcı Tercihi Kalıcı Hale Getirildi**: `Değerlendir` seçilirse Microsoft Store değerlendirme sayfası açılır ve tekrar sorulmaz. `Sonra` seçilirse popup 3 gün ertelenir. `Bir daha sorma` seçilirse tekrar gösterim tamamen kapatılır.
- [Yeni] **Microsoft Store Review Link Akışı**: Store değerlendirme linki `NoctraStoreProductId` veya `NoctraStoreReviewLaunchUri` metadata alanlarından üretilir. Bu alanlar Premium satın alma ProductId'sinden ayrıdır. Store paketi içinde ProductId verilmemişse `PFN` fallback'i denenir; debug/unpackaged ortamda placeholder veya bozuk Store linki kullanıcıya gösterilmez.
- [Yeni] **Review Prompt Servis İzolasyonu**: `IReviewPromptService` ve Avalonia tarafında `ReviewPromptService` eklendi. Ana pencere kapanırsa bekleyen gecikmeli prompt task'i cancellation token ile iptal edilir; eski pencere/profil yaşam döngüsünden dialog sızması engellenir.
- [Yeni] **ReviewPromptWindow**: Mevcut iki butonlu `DialogWindow` bozulmadan, değerlendirme akışı için ayrı küçük modal pencere eklendi. Butonlar: `Değerlendir`, `Sonra`, `Bir daha sorma`.
- [Lokalizasyon] **Tüm Dillerde Review Prompt Metinleri**: `tr-TR`, `en-US`, `de-DE`, `fr-FR`, `es-ES` çeviri dosyalarına prompt başlığı, açıklaması, butonları ve Microsoft Store açılamadı hata metinleri eklendi.
- [Paketleme] **Store Profil Metadata Alanları**: `Directory.Build.props`, `Noctra.Avalonia.csproj`, `build/package-store.ps1` ve `Noctra.Packaging/store-profiles.json` Store review ProductId/URI metadata akışını destekleyecek şekilde güncellendi.
- [Dokümantasyon] **CHANGELOG Güncellendi**: Ana `CHANGELOG.md` ve legacy changelog, review prompt davranışı, 3 günlük erteleme standardı ve Store metadata ayrımıyla güncellendi.
...
- [Düzeltildi] **Stalker Büyük Playlist Resume ve Dummy Series Takılması**: Stalker progressive load sırasında sorunlu bir kategori artık sonsuza kadar progress'i dondurmuyor; kategori başı timeout sonrası dummy temizlenip kuyruktaki sonraki kategoriye geçiliyor. Resume filtresi yalnızca group adıyla değil `stalker-dummy://<categoryId>` içindeki provider kategori ID'siyle de eşleşiyor. Yarım yüklemeden kalan `İçerik yükleniyor` Series aggregate kayıtları resume başında temizleniyor ve dummy kanallar Series tablosuna tekrar yazılmıyor.
- [Düzeltildi] **Büyük Playlist Yüklerken UI Donması**: Cached Xtream/Stalker resume artık UI thread üzerinde başlamıyor. Playlist yüklenirken `OnAggregationCompleted` kaynaklı Home/Series yenilemeleri erteleniyor; bu sayede arka plan kategori yüklemesi sürerken pencere müdahalesiz/donmuş hale gelmiyor.
- [Düzeltildi] **Progress Mesajının Channel List Ready ile Ezilmesi**: Background aggregation event'i aktif provider progress varken status mesajını `Channel List Ready ✓` olarak değiştirmiyor. Stalker/Xtream progress metni yükleme bitene kadar korunuyor.
- [Düzeltildi] **Status Bar Çift Progress Mesajı**: Kanal yükleme progress metni hem `StatusMessage` hem `ChannelLoadingStats` olarak aynı anda basılmıyor; altta aynı metnin iki kez görünmesi engellendi.
...
- [Kaldırıldı] **Çift Hesap Kısıtlaması**: Aynı M3U, Xtream Codes veya Stalker Portal sağlayıcısının farklı profillerde kullanılmasını engelleyen kısıtlama tamamen kaldırıldı. Kod ve localization dosyalarından tüm izleri temizlendi.
...
- [Düzeltildi] **Provider Series Posterleri Liste Cevabından Okunuyor**: Stalker Series kategori liste cevabındaki `screenshot_url`, `icon`, `movie_image`, `cover_big`, `poster`, `poster_url` ve `image` alanları da kart poster adayı olarak okunuyor. Xtream/Stalker kart listesi yavaş per-card provider detail poster çağrısı yapmaz; TMDB görünür kart zenginleştirmesi M3U ile sınırlı kalır.
- [Düzeltildi] **Stalker VOD Posterlerinde Boş `pic` Fallback Hatası**: Gerçek Stalker provider cevabında VOD listesinde `pic` alanı boş string (`""`) gelirken `screenshot_uri` dolu poster URL'si dönebiliyordu. Eski `pic ?? screenshot_uri` zinciri boş string'i geçerli kabul ettiği için poster `LogoUrl` alanına hiç düşmüyor ve VOD kartları gri kalıyordu. Stalker parser artık `FirstNonBlank` kullanarak `logo`, `pic`, `cover`, `screenshot_uri`, `screenshot_url`, `icon`, `movie_image`, `cover_big`, `poster`, `poster_url`, `image_url`, `thumbnail` ve `image` alanları arasında ilk dolu değeri seçiyor.
- [Düzeltildi] **Stalker Series ve Episode Görsel Fallbackleri**: Stalker Series detail ve episode parse akışında `pic`, `screenshot_uri`, `icon`, `cover`, `movie_image`, `screenshot_url` ve parent Series cover fallback zinciri boş string'leri atlayacak şekilde güncellendi. Episode küçük resmi yoksa sezon/dizi cover'ına düşme davranışı korunur.
- [Düzeltildi] **Xtream Series ve Episode Görsel Fallbackleri**: Xtream VOD liste kartlarında `FirstNonEmpty` zaten kullanılıyordu; aynı boş-string riski Series detail, Series list, season ve episode parse taraflarında da kapatıldı. Series posterleri artık `cover`, `stream_icon`, `cover_big`, `movie_image`, `poster`, `poster_url`, `image`, `screenshot_uri`, `screenshot_url` alanları arasında ilk dolu değeri kullanır. Episode küçük resimleri `stream_icon`, `icon`, `cover`, `info.movie_image`, `info.cover`, `info.screenshot_uri`, `info.image` ve en son Series cover fallback'iyle doldurulur.
- [Temizlik] **Stalker Görsel Enrichment Sınırı Kaldırıldı**: Gerçek kök neden parser fallback olduğu için Stalker için eklenen 100 group sınırı, `StalkerVisualEnrichmentPolicy`, policy testleri ve visible-card provider-detail enrichment kodları kaldırıldı. Stalker/Xtream artık provider liste metadata'sına güvenir; TMDB visible-card enrichment yalnızca M3U profillerde çalışır.
...
- [Değişti] **Provider Şifreleri UI'da Maskelenmiyor**: Add/Edit Profile ekranındaki provider password alanında `PasswordChar` kaldırıldı; PIN alanları maskeli kalmaya devam eder. Settings ekranında provider password artık `**********` olarak değil, `ISecurityService` ile çözülen gerçek değer olarak gösterilir.
- [Değişti] **Settings Account Information Provider Tipine Göre Düzenlendi**: M3U profillerde yalnız `M3U Link`, Xtream profillerde `Server URL`, `Username`, `Password`, `Expiry`, Stalker profillerde `Server URL`, `MAC Address`, `Expiry` gösterilir. SettingsViewModel'e provider label ve görünürlük property'leri eklendi.
- [Düzeltildi] **Uzun M3U Link Settings Kartından Taşmıyor**: M3U playlist URL'si Settings Account Information bölümünde input gibi görünmeden normal TextBlock olarak satıra kırılır. Label kolonu daraltıldı ve value alanı genişletildi; uzun linkler kart içinde okunabilir şekilde sarılır.
...
- [Doğrulama] `dotnet build NoctraPlayer.sln` başarılı. `dotnet test Noctra.Tests\Noctra.Tests.csproj --no-build` sonucu `906/906` test geçiyor. `git diff --check` yalnızca CRLF uyarıları veriyor.
...
- [Düzeltildi] **M3U Series Ekranı Boş Kalma Sorunu**: M3U playlistlerde `Channels` içinde `Series` kayıtları olduğu halde arka plan aggregation kaçmış veya eski importtan `Series` tablosu boş kalmışsa Series ekranı artık kendini onarır. Sorunun nedeni, M3U importunun önce `Channels` tablosunu doldurması; Series ekranının ise doğrudan `Channels` yerine aggregation sonrası oluşan `Series` tablosundan okumasıydı. Background aggregation fire-and-forget çalıştığı için hata, erken ekran geçişi veya eski import durumunda `Series` tablosu boş kalabiliyordu. `LoadHomeContentAsync`, boş series cache durumunda playlistte `ChannelType.Series` var mı kontrol eder, aggregation'ı bekleyerek çalıştırır ve Series listesini tekrar yükler.
- [Düzeltildi] **Kanal Listesi Yenileme Tam Replacement Oldu**: `RefreshAsync` ve aşamalı provider refresh akışları yeni listeyi başarıyla parse/organize ettikten sonra eski playlist içeriğini komple temizler; eski `Channels`, playlist `Series/Season/Episode` metadata'sı ve eski EPG eşleşmeleri silindikten sonra yeni içerik yazılır. Kaynak hata verirse veya boş dönerse eski playlist korunur.
- [Düzeltildi] **Profil Provider Değişikliği Veri Kaybına Karşı Korundu**: Profil düzenlemede URL/kullanıcı/parola/provider tipi değişirse kayıt öncesi yeni provider preview ile doğrulanır. Yeni provider çalışmıyor veya içerik döndürmüyorsa eski profil bilgisi ve eski playlist DB verisi silinmez.
- [Düzeltildi] **Series İzleme Durumu Yeni Provider'a Taşınıyor**: İzlenen, tamamlanan ve yarım kalan dizi bölümleri `SeriesEpisodeProgress` üzerinden profil bazlı korunur. Yeni playlist/provider sonrası Series detay ekranı, History ve Continue Watching artık eski episode ID'lerine bağlı kalmadan dizi adı anahtarı + sezon/bölüm eşleşmesiyle bu ilerlemeyi geri uygular.
- [Düzeltildi] **Series HD/FHD/Dil Varyantları Aynı Progress'i Kullanıyor**: `TR | Breaking Bad FHD`, `EN | Breaking Bad HD` gibi aynı dizinin dil veya kalite farkı olan varyantlarında sezon/bölüm ilerlemesi aynı normalize series key üzerinden uygulanır. VOD içerikler için provider değişiminde özel progress taşıma yapılmaz.
- [Test] **Refresh ve Provider Değişikliği Regression Testleri**: Playlist refresh'in eski series/EPG kalıntılarını temizlediği, credential değişikliğinde playlist içeriğinin silinip profile-wide `SeriesEpisodeProgress` bilgisinin korunduğu test edildi.
...
- [Duzeltildi] **Xtream/Stalker Series Linkleri Korundu**: Provider'dan `xtream-series://` veya `stalker-series://` olarak gelen gercek dizi kayitlari mevcut-DB repair akisi tarafindan Live'a cevrilmez; Xtream/Stalker tip ayrimi provider endpoint'lerine gore korunur.
...
- [Duzeltildi] **Uzantisiz M3U Film Proxy URL'leri**: Bazi M3U provider'larinin film satirlarini `.mp4/.mkv` veya `/movie/` olmadan proxy URL ile vermesi nedeniyle filmler Live TV'de gorunebiliyordu. Lineer stream olmayan ve basliginda yil bilgisi olan film kayitlari artik VOD olarak siniflandirilir.
- [Duzeltildi] **Mevcut Live'a Dusmus Film Kayitlari Onarimi**: Daha once Live olarak kaydedilmis, yil bilgili ve lineer olmayan M3U film kayitlari playlist okunurken VOD'a onarilir.
...
- [Performans] **DB ve Kanal Liste Sorgulari Optimize Edildi**: Filtreli kanal sorgularinda `AsNoTracking`, hedefli paging ve kanal tipi/grup/playlist indeks fixup'lari kullanildi. Linear stream tip onarimi servis basina tek seferlik bulk SQL akisine tasindi.
- [Performans] **Image Warmup Sinirlandi**: Gorunur/current rail odakli preload yapisi kullanildi; image concurrency, cache boyutu, timeout ve retry baskisi dusuruldu.
- [Duzeltildi] **Kart Gorsellerinin Placeholder'da Kalmasi**: `RemoteImage`, recycle/detach sonrasi ayni URL tekrar atanmadiginda bile attach ve data-context degisimlerinde mevcut URL'yi yeniden deniyor. Cache'e inen gorselin ekrana uygulanmasi `RemoteImage source-applied` loguyla izleniyor.
- [Duzeltildi] **History ve Episode Kartlarinda Cache Uygulama Yarisi**: Loglarda `Series`/`Episode` icin `cover=set` oldugu halde kart bos kalabiliyordu. `RemoteImage` artik attach/data-context/await anlarinda memory cache'i hemen uygular, source set islemlerini render onceligine tasir ve `cache-notify` ayni URL'li recycle edilmis kartlari yeniden boyar. Loglara `reason=memory-cache`, `reason=retry-*` ve `reason=await-cache` ayrimi eklendi.
- [Duzeltildi] **Preload Cache Yarisi**: `RemoteImage.PreloadAsync` gorseli cache'e indirirken kart recycle/cancel olursa aktif kartlar artik cache doldugunda `reason=cache-notify` ile uyandirilir. Ilk acilista Continue Watching / Series / VOD kartlarinin ikinci acisi beklemeden gorseli almasi hedeflendi.
- [Duzeltildi] **Kalici Image Cache**: Basarili indirilen HTTP gorselleri `%LOCALAPPDATA%/Noctra/ImageCache` altina yazilir; uygulama kapanip acildiginda ayni URL icin tekrar download beklemeden `RemoteImage disk-cache-hit` ile yuklenir.
- [Duzeltildi] **Kart Gorselinin Sonradan Kaybolmasi**: Memory cache LRU eviction artik `Bitmap.Dispose()` cagirmiyor; aktif kartlarin halen kullandigi bitmap dispose edilip ekranda bos gorunme riski kaldirildi. Eviction `RemoteImage cache-evict` loguyla izlenir.
- [Duzeltildi] **Ilk Acilista Gorsel Kuyrugu Gec Baslamasi**: Kart basina image load ertelemesi 90ms'den 20ms'ye, gorunur ekran warmup gecikmesi 350ms'den 100ms'ye indirildi. Ilk gorunen live logo, VOD poster ve series posterleri daha erken download/cache kuyruguna girer.
- [Duzeltildi] **Bozuk Provider Posterlerinin TMDB ile Degistirilmesi**: M3U VOD/Series gorsellerinde URL dolu olsa bile TMDB disi provider host'u kullaniliyorsa lazy visual enrichment artik bunu aday sayar ve TMDB poster URL'siyle yeniler. `QueueVisible*VisualEnrichment` ve `Enrich*VisualAsync poster` loglari aday sayisini ve eski/yeni host bilgisini yazar.
- [Duzeltildi] **Bilinen Bozuk Gorsel Hostlari Bekletmiyor**: `nox101.com` ve `logo.uixtreamreseller.com` gorselleri artik 5 saniyelik HTTP timeout slotlarini doldurmaz; `RemoteImage known-bad-host skipped` / `skip-known-bad-host` loglariyla direkt atlanir ve Series/VOD tarafinda TMDB fallback akisi devreye girer.
- [Log] **TMDB Fallback Sonucu Ayrildi**: Lazy VOD/Series gorsel zenginlestirme metadata/poster bulamazsa `EnrichChannelVisualAsync no-poster` ve `EnrichSeriesVisualAsync no-poster` satirlari yazilir.
- [Log] **TMDB Arama Sebebi Ayrildi**: Series search fallback artik eksik API key, HTTP hata, sonuc yok, poster path yok ve fallback-hit durumlarini `TMDB | SearchSeriesAsync ...` satirlariyla ayirir.
- [Duzeltildi] **TMDB v3/v4 Auth Uyumlulugu**: M3U profillerde TMDB zenginlestirme icin `TMDB_API_KEY` 32 haneli v3 key ise `api_key` query auth, `TMDB_BEARER_TOKEN` veya `SetApiKey` ise Bearer auth kullanilir. `401` loglarina `authMode` eklendi.
...
- [Duzeltildi] **Update Manifest Repo Adresi Duzeltildi**: `UpdateService` manifest URL'si `Galdr1c/NoctraPlayer/main/update.json` adresine tasindi ve repo kokune uyumlu `update.json` manifest dosyasi eklendi.
...
- [Düzeltildi] **Çoklu EPG Kaynağı Tekilleştirme**: Custom EPG, provider EPG ve M3U header EPG birlikte kullanıldığında aynı kanal/saat/program kayıtları artık ikinci kez kaydedilmez. Eski duplicate kayıtlar DB'de kalsa bile EPG okuma tarafında tekilleştirilerek timeline'da üst üste program blokları oluşması engellenir.
- [Düzeltildi] **Çakışan Farklı EPG Başlıkları Normalize Edildi**: Aynı kanal ve zaman aralığında farklı kaynaklardan gelen farklı program başlıkları artık okuma sırasında çakışmasız timeline'a dönüştürülür. Overlay'deki aktif program ve EPG panelindeki `ŞİMDİ` çizgisi aynı normalize edilmiş programı gösterir.
...
- [Duzeltildi] **Resume Sonrasi Slider 00:00'da Donma Sorunu**: Continue Watch'ta 'Devam Et' tiklandiginda video kaldigi yerden basliyor ancak TimelineSlider 00:00'da donup ilerlemiyordu. Kok neden: TryApplyPendingResumeSeek metodu hicbir yerden cagrilmiyordu (orphaned method). _pendingResumeSeekPosition > 1 tum PositionChanged guncellemelerini bloke ediyor ama blokaji kaldiracak kod hic calismiyordu. PlayerPlaybackController.OnVideoPlayerServicePlayingChanged icine TryApplyPendingResumeSeek() cagrisi ve OnVideoPlayerServicePositionChanged icine safety net (hedefe 3s yakinsa blokaji otomatik kaldir) eklendi.
...
- **Live TV Sürekli Kopma Döngüsü Çözüldü** (2026-03-21):
  - **Dinamik Kurtarma Süresi:** Sağlayıcı veya internet kaynaklı her ~50 saniyede bir kopan canlı yayınların, 5 deneme limitine takılarak tamamen durması sorunu çözüldü. Artık yayın 30 saniyeden fazla sorunsuz oynatıldığında kurtarma sayacı sıfırlanarak, kullanıcının müdahalesine gerek kalmadan sonsuz ve şeffaf bir yeniden bağlanma deneyimi sunuluyor.
  - **Kesintisiz IPTV Akışı (Tivimate/Smarters Deneyimi):** Kötü huylu IPTV sunucularının veya anlık ağ dalgalanmalarının oynatıcıyı tamamen durdurması (EOF) engellendi. `LiveTs` formatları için `http-continuous` modülü aktif edildi ve ağ tamponu (network-caching) artırıldı. Bu sayede sunucu milisaniyelik veriyi kestiğinde video kapanıp yeniden açılmak (`PlayAsync` restarting) yerine, diğer popüler uygulamalarda olduğu gibi arkada tamponlayarak (buffer) şeffaf şekilde, fark edilmeden kaldığı yerden devam edecek.
  - **Sessiz Donmalara Karşı Kalkan (Timeout Önlemi):** Bazı sunucuların bağlantıyı tamamen açık tutup hiç veri göndermemesi durumunda oynatıcının sonsuza kadar "Siyah Ekranda/Donuk" kalmasını önlemek için sisteme 12 saniyelik agresif bir `ipv4-timeout` (Soket Zaman Aşımı) eklendi. Artık sunucu kilitlenirse oynatıcı durumu anında algılayıp yeni bağlantı talebi oluşturacak.
  - **Thread-safe Loglama:** Farklı thread'lerin aynı anda `vlc_debug_log.txt` dosyasına yazmaya çalışmasından kaynaklı sessiz hatalar (log kayıpları) giderildi. `LogDebug` metoduna `lock` mekanizması eklendi.
  - **Pozisyon Önbellek Sızıntısı:** VOD/Dizi izlerken kaydedilen `_lastKnownValidPosition` değerinin, hemen ardından açılan Canlı TV yayınlarına hatalı şekilde (örn. 414s) sızması sorunu düzeltildi. Sayacın her yeni yayında temizlenmesi sağlandı.
