...
- [Düzeltildi] **Stalker Series Posterleri Detay Açmadan Doluyor**: Stalker Series kategori liste cevabındaki `screenshot_url`, `icon`, `movie_image`, `cover_big`, `poster`, `poster_url` ve `image` alanları da kart poster adayı olarak okunuyor. Böylece poster bilgisi listede geldiği halde kaçırılan diziler detay ekranı açılmadan resimlenebilir; Xtream/Stalker için görünür liste TMDB zenginleştirmesi kapalı kalır.
