...
- **Real trial detection end-to-end**: the billing backend now reads the active offer from `subscriptionsv2.get` (`lineItems[].offerId`) and cross-checks it against the Google Play Monetization API product details (`pricingPhases.pricingPhaseList[].recurrenceMode`): when the active offer contains a **free** `NON_RECURRING` phase (`priceAmountMicros == 0`), the verification carries an `IsTrialPeriod` flag through `VerifiedEntitlementResponse`, the client (`BillingVerifiedEntitlement` → `StoreEntitlement`) and into `LicenseService`, which now marks `SubscriptionInfo.IsTrialPeriod` only for a real trial — a paying monthly user can never appear as a trial, and a paid introductory offer (non-recurring but not free) is not treated as a trial either. Trial detection is best-effort (product details unavailable → false, entitlement is never blocked, cancellation is propagated). The `EntitlementStore` schema gained `is_trial_period` with an automatic migration for legacy tables.
...
- **RTDN honors the Pub/Sub retry contract**: the push endpoint no longer always returns `200` after catching errors — only genuinely processed or malformed/irrelevant envelopes get a 2xx acknowledge; transient failures (Play API 503, network) now return `503` so Pub/Sub redelivers instead of silently dropping the notification. **OIDC push authentication**: every RTDN request's `Authorization` bearer token is verified against Google's public JWKS (RS256, `OidcTokenVerifier`) — `aud`, `iss`, `exp`, and now also `email` + `email_verified` (the push subscription's signing service-account email must match `NOCTRA_RTDN_SERVICE_ACCOUNT_EMAIL`); wrong/missing tokens get `401` and are never processed; unverified requests are fail-closed. JWKS keys are cached for 1 h, an unknown `kid` (Google key rotation) invalidates the cache and refetches at most once per 30 s to survive rotation without allowing token floods to hammer the network.
- **RTDN OIDC is now fail-fast**: `NOCTRA_RTDN_AUDIENCE` and `NOCTRA_RTDN_SERVICE_ACCOUNT_EMAIL` are required at startup — the backend refuses to boot with RTDN authentication silently disabled in production. RTDN can still be explicitly turned off with `NOCTRA_RTDN_DISABLED=1`. Both deploy scripts read the values from `.env` and fail with clear instructions when missing.
- **RTDN no longer depends on Google's deprecated fields**: `SubscriptionNotification.subscriptionId` and `OneTimeProductNotification.sku` are deprecated by Google (and will be absent from new payloads), so `ReverifyByTokenAsync(purchaseToken)` now derives the product IDs from the stored rows matching the purchase-token hash instead of trusting the notification — subscription renewals/cancellations/grace-period/on-hold/pause/revoke/expiry RTDNs are processed again (the old code silently skipped them). Only server-configured product IDs are verified (a stale row for a retired product never triggers a Play request that would cause an endless Pub/Sub retry), and multi-installation rows sharing the token are all updated. The RTDN handler also validates `payload.packageName` against the server config before processing, handles `voidedPurchaseNotification` (refunds/chargebacks re-verify against Play), and the one-time-product `notificationType` doc now matches Google's real enum (1 = PURCHASED, 2 = CANCELED). **One-time product notifications** (`oneTimeProductNotification`, lifetime purchases) are now processed through the same re-verification path as subscriptions. **Multi-device RTDN**: re-verification now updates **every** installation row sharing the purchase-token hash (`GetAllByPurchaseTokenHashAsync`) — a purchase restored on phone + tablet no longer leaves the tablet on a stale entitlement. A deliberate-scope note documents that the file-based SQLite store is not persistent on Cloud Run (ephemeral filesystem); the core client flow is unaffected because verification queries Play on every request, but a shared persistent store (Firestore / Cloud SQL / Supabase) is planned for production.
