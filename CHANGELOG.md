...
- **Kapsamlı Servis Refaktörü ve Service Locator Arındırma** (2026-02-21 12:36):
  ...
  - **Güvenlik: DPAPI Şifre Koruması**: `ISecurityService` arayüzü ve `SecurityService` implementasyonu eklendi. Xtream Codes ve Stalker Portal şifreleri artık SQLite'a düz metin olarak yazılmak yerine, Windows DPAPI (`ProtectedData.Protect/Unprotect`) ile şifrelenerek `CurrentUser` kapsamında korunuyor. `AddProfileViewModel` kaydetme/düzenleme sırasında şifreler otomatik olarak encrypt/decrypt edilir. `MainViewModel` içerisinde playlist yükleme ve expiration kontrolü sırasında da şifreler çözülerek servis çağrılarına aktarılır.
  ...
  - **Kapsamlı Unit Test Paketi**: `SeriesInfoParser` için 55 adet xUnit testi eklendi. Testler çok dilli kalıpları (İngilizce S01E05, Türkçe Sezon/Bölüm, İspanyolca Temporada, Fransızca Saison, Almanca Staffel/Folge), pipe-etiketli isimleri, null/boş girdi güvenliğini, `IsSeries` doğruluğunu ve `NormalizeKey` tutarlılığını kapsar.
