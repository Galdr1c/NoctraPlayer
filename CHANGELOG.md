...
- **Performans Optimizasyonu ve Modernizasyon** (2026-02-20):
    - **Uygulama Açılış Hızı Çözüldü (Kritik)**: Uygulamayı başlatırken UI thread'ini 10-15 saniye kilitleyen ağır senkron `LibVLCSharp.Shared.Core.Initialize()` çağrısı kaldırıldı. Özelliğin asenkron olarak arka plan görevinde başlatılması sağlandı ve uygulamanın UI'si anında render edilebilir (Instant Load) duruma getirildi. `MainWindow`, arka planda VLC yüklendiğinde `MediaPlayerReady` üzerinden otomatik olarak ekrana bağlandı.
