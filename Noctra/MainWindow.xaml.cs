...
public partial class MainWindow : Window
{
    ...
    {
        if (!e.Handled || PlayerArea.Visibility == Visibility.Visible)
        {
            return;
        }

        var activeScrollViewer = GetActiveScrollViewer();
        if (activeScrollViewer == null)
        {
            return;
        }

        var steps = Math.Max(1, Math.Abs(e.Delta) / Mouse.MouseWheelDeltaForOneLine);
    ...
    {
        return _viewModel.ActiveView switch
        {
            AppView.Home => HomeView,
            AppView.Movies => MoviesView,
